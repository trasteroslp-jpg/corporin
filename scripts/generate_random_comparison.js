...
readXlsxFile(filePath).then(async (rows) => {
    ...
    for (let i = 1; i < rows.length - 1; i++) {
        if (rows[i][originIdx] === 'Cliente' && rows[i+1][originIdx] === 'Agente') {
            interactions.push({ 
                user: String(rows[i][msgIdx] || ""), 
                oldBot: String(rows[i+1][msgIdx] || "") 
            });
        }
    }

    // Seleccionamos 5 aleatorias
    const shuffled = interactions.sort(() => 0.5 - Math.random());
    const selected = shuffled.slice(0, 5);

    console.log(`Processing 5 random cases...`);
    const results = [];
    ...
    fs.writeFileSync('random_comparison_5.json', JSON.stringify(results, null, 2));
    console.log("Random comparison results saved to random_comparison_5.json");
